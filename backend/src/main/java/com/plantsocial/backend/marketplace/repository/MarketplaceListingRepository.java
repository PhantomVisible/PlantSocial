...
public interface MarketplaceListingRepository extends JpaRepository<MarketplaceListing, UUID> {
    ...
    boolean existsByUser_IdAndStatusAndTitleIgnoreCase(UUID userId, ListingStatus status, String title);

    boolean existsByUser_IdAndStatusAndProductUrl(UUID userId, ListingStatus status, String productUrl);
